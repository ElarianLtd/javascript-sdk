...
const { RSocketClient, BufferEncoders, RSocketResumableTransport } = require('rsocket-core');
...
const connectRSocket = (options) => new Promise((resolve, reject) => {
    const {
        orgId,
        appId,
        apiKey,
    ...
    const transport = new RSocketResumableTransport(
        () => new RSocketTcpClient({ host, port }, BufferEncoders),
        {
            ...
            resumeToken: Buffer.from(appId), // unique identifier the session across connections
            sessionDurationSeconds,
        },
        BufferEncoders,
    );

    const metadata = new AppConnectionMetadata();
    metadata.getEntriesMap()
        .set('org-id', new DataMapValue().setBytesVal(Buffer.from(orgId)))
        .set('app-id', new DataMapValue().setBytesVal(Buffer.from(appId)))
        .set('api-key', new DataMapValue().setBytesVal(Buffer.from(apiKey)));
    const client = new RSocketClient({
        ...
        setup: {
            ...
            metadataMimeType,
            payload: {
                metadata: Buffer.from(metadata.serializeBinary()),
            },
        },
        responder: {
    ...
    transport.connectionStatus().subscribe({
        onNext: (status) => {
            log.debug('Transport Status: ', status.kind);
            if (status.kind === 'NOT_CONNECTED') {
                if (!started) {
                    setTimeout(() => transport.connect(), reconnectIntervalMillis);
                } else {
                    started = false;
                }
            }
