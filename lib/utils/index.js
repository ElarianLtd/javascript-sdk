...
const { RSocketClient, BufferEncoders, RSocketResumableTransport, BufferEncoder } = require('rsocket-core');
...
const connectRSocket = (options) => new Promise((resolve, reject) => {
    ...
    const transport = new RSocketTcpClient({ host, port }, BufferEncoders);

    const metadata = new AppConnectionMetadata();
    metadata.getEntriesMap()
        .set('org-id', new DataMapValue().setStringVal(orgId))
        .set('app-id', new DataMapValue().setStringVal(appId))
        .set('api-key', new DataMapValue().setStringVal(apiKey));
    const client = new RSocketClient({
        ...
        setup: {
            ...
            payload: {
                data: Buffer.from(metadata.serializeBinary()),
            },
        },
        responder: {
