...
function Customer(params) {
    ...
    this.getState = async () => {
        ...
        if (state.status) {
            ...
            const numbers = [
                ...(state.data.messagingState ? state.data.messagingState.customerNumbersList : []).map((i) => ({ ...i, state: 'messaging' })),
                ...(state.data.ussdState ? state.data.ussdState.customerNumbersList : []).map((i) => ({ ...i, state: 'ussd' })),
                ...(state.data.voiceState ? state.data.voiceState.customerNumbersList : []).map((i) => ({ ...i, state: 'voice' })),
                ...(state.data.paymentState ? state.data.paymentState.customerNumbersList : []).map((i) => ({ ...i, state: 'payment' })),
            ];

            if (numbers.length && !this.customerNumber) {
                this.customerNumber = numbers.find((i) => i.number);
            }
...
const getCustomerState = async (customer) => {
    try {
        ...
        const res = await stub.getCustomerState(req, extraHeaders);
        const result = {
...
const adoptCustomerState = async (customer, otherCustomer) => {
    try {
        ...
        const res = await stub.adoptCustomerState(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
...
const updateCustomerTag = async (customer, tags) => {
    try {
        ...
        const res = await stub.updateCustomerTag(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const deleteCustomerTag = async (customer, tags) => {
    try {
        ...
        const res = await stub.deleteCustomerTag(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const updateCustomerSecondaryId = async (customer, secondaryIds) => {
    try {
        ...
        const res = await stub.updateCustomerSecondaryId(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const deleteCustomerSecondaryId = async (customer, secondaryIds) => {
    try {
        ...
        const res = await stub.deleteCustomerSecondaryId(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const addCustomerReminder = async (customer, reminder) => {
    try {
        ...
        const res = await stub.addCustomerReminder(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const cancelCustomerReminder = async (customer, key) => {
    try {
        ...
        const res = await stub.cancelCustomerReminder(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const addCustomerReminderByTag = async (tag, reminder) => {
    try {
        ...
        const res = await stub.addCustomerReminderByTag(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
...
const cancelCustomerReminderByTag = async (tag, key) => {
    try {
        ...
        const res = await stub.cancelCustomerReminderByTag(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            workId: (res.getWorkId() || {
...
const updateCustomerMetadata = async (customer, metadata) => {
    try {
        ...
        const res = await stub.updateCustomerMetadata(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const leaseCustomerMetadata = async (customer, key) => {
    try {
        ...
        const res = await stub.leaseCustomerMetadata(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
...
const deleteCustomerMetadata = async (customer, keys) => {
    try {
        ...
        const res = await stub.deleteCustomerMetadata(req, extraHeaders);
        const result = {
            status: res.getStatus(),
            description: res.getDescription(),
            customerId: (res.getCustomerId() || {
